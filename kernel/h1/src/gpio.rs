...
pub struct GPIOPin {
    ...
    pin: Pin,
    change: Cell<bool>,
    interrupt_client: Cell<Option<&'static dyn hil::gpio::Client>>,
}
...
impl GPIOPin {
    const fn new(port: *mut PortRegisters, pin: Pin) -> GPIOPin {
        GPIOPin {
            ...
            interrupt_client: Cell::new(None),
        }
    }
    ...
    pub fn handle_interrupt(&self) {
        ...
        if let Some(client) = self.interrupt_client.get() {
            client.fired();
        }
    }
...
impl hil::gpio::Interrupt for GPIOPin {
    fn set_client(&self, client: &'static dyn hil::gpio::Client) {
        self.interrupt_client.set(Some(client));
    }
